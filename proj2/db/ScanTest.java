 ...
 */
public class ScanTest extends DbTestBase {
    ...
    private void validateScan(int[] columnSizes, int[] rowSizes)
        ...
        for (int columns : columnSizes) {
            for (int rows : rowSizes) {
                ...
                HeapFile f = SystemTestUtil.createRandomHeapFile(columns, rows, null, Rows);
                SystemTestUtil.matchRows(f, Rows);
    ...
    @Test public void testRewind() throws IOException, DbException, TransactionAbortedException {
        ArrayList<ArrayList<Integer>> Rows = new ArrayList<ArrayList<Integer>>();
        HeapFile f = SystemTestUtil.createRandomHeapFile(2, 1000, null, Rows);
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            Row t = scan.next();
            assertEquals(Rows.get(i), SystemTestUtil.RowToList(t));
        }
    ...
    @Test public void testCache() throws IOException, DbException, TransactionAbortedException {
        ...
        class InstrumentedHeapFile extends HeapFile {
            public InstrumentedHeapFile(File f, RowDesc td) {
                super(f, td);
            }
        ...
        final int PAGES = 30;
        ArrayList<ArrayList<Integer>> Rows = new ArrayList<ArrayList<Integer>>();
        File f = SystemTestUtil.createRandomHeapFileUnopened(1, 992*PAGES, 1000, null, Rows);
        RowDesc td = Utility.getRowDesc(1);
        InstrumentedHeapFile table = new InstrumentedHeapFile(f, td);
        Database.getCatalog().addTable(table, SystemTestUtil.getUUID());

        // Scan the table once
        SystemTestUtil.matchRows(table, Rows);
